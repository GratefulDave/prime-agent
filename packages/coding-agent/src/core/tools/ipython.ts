...
const ipythonSchema = Type.Object({
	code: Type.String({
		description:
			"Python code to execute in the persistent Python REPL. Spawn an independent child with `handle = await rlm('task', name='worker')`; it returns at admission, not the child's answer. Use the target project's own environment for project imports, tests, scripts, CLIs, and dependency checks instead of direct kernel imports.",
	}),
});
